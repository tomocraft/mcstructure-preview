...
var TextNode = class _TextNode extends Text {
  ...
  scanIdentical(other, dir2) {
    ...
    for (; ; iA += dir2, iB += dir2) {
      ...
      if (chA != chB)
        return length + chA.scanIdentical(chB, dir2);
      length += chA.length + 1;
    }
  }
...
var RawTextCursor = class {
  constructor(text, dir2 = 1) {
    this.dir = dir2;
    this.done = false;
    this.lineBreak = false;
    this.value = "";
    this.nodes = [text];
    this.offsets = [dir2 > 0 ? 1 : (text instanceof TextLeaf ? text.text.length : text.children.length) << 1];
  }
  nextInner(skip, dir2) {
    ...
    for (; ; ) {
      ...
      if (offset == (dir2 > 0 ? size : 0)) {
        ...
        let next = top2.text[offset + (dir2 < 0 ? -1 : 0)];
        this.offsets[last] += dir2;
        ...
        let next = top2.children[offset + (dir2 < 0 ? -1 : 0)];
        if (skip > next.length) {
          skip -= next.length;
          this.offsets[last] += dir2;
        } else {
...
var PartialTextCursor = class {
  ...
  nextInner(skip, dir2) {
    ...
    this.pos += (value.length + skip) * dir2;
    this.value = value.length <= limit ? value : dir2 < 0 ? value.slice(value.length - limit) : value.slice(0, limit);
...
var BidiSpan = class {
  ...
  */
  ...
  @internal
  */
  forward(forward, dir2) {
    return forward == (this.dir == dir2);
  }
  /**
  @internal
  */
...
var movedOver = "";
function moveVisually(line, order, dir2, start, forward) {
  var _a2;
  let startIndex = start.head - line.from;
  let spanI = BidiSpan.find(order, startIndex, (_a2 = start.bidiLevel) !== null && _a2 !== void 0 ? _a2 : -1, start.assoc);
  let span = order[spanI], spanEnd = span.side(forward, dir2);
  if (startIndex == spanEnd) {
    let nextI = spanI += forward ? 1 : -1;
    if (nextI < 0 || nextI >= order.length)
      return null;
    span = order[spanI = nextI];
    startIndex = span.side(!forward, dir2);
    spanEnd = span.side(forward, dir2);
  }
  let nextIndex = findClusterBreak2(line.text, startIndex, span.forward(forward, dir2));
  if (nextIndex < span.from || nextIndex > span.to)
    nextIndex = spanEnd;
  movedOver = line.text.slice(Math.min(startIndex, nextIndex), Math.max(startIndex, nextIndex));
  let nextSpan = spanI == (forward ? order.length - 1 : 0) ? null : order[spanI + (forward ? 1 : -1)];
  if (nextSpan && nextIndex == spanEnd && nextSpan.level + (forward ? 0 : 1) < span.level)
    return EditorSelection.cursor(nextSpan.side(!forward, dir2) + line.from, nextSpan.forward(forward, dir2) ? 1 : -1, nextSpan.level);
  return EditorSelection.cursor(nextIndex + line.from, span.forward(forward, dir2) ? -1 : 1, span.level);
}
...
function moveVertically(view, start, forward, distance) {
  let startPos = start.head, dir2 = forward ? 1 : -1;
...
function findChild(tile, dom, dir2) {
  while (dom) {
    ...
    dom = parent != tile.dom ? parent : dir2 > 0 ? dom.nextSibling : dom.previousSibling;
  }
  return null;
}
...
var EditorView = class _EditorView {
  ...
  */
  visualLineSide(line, end) {
    ...
    return EditorSelection.cursor(span.side(end, dir2) + line.from, span.forward(!end, dir2) ? 1 : -1);
  }
  /**
...
var CachedOrder = class _CachedOrder {
  constructor(from, to, dir2, isolates, fresh, order) {
    this.from = from;
    this.to = to;
    this.dir = dir2;
...
var TreeNode = class _TreeNode extends BaseNode {
  ...
  nextChild(i, dir2, pos, side, mode = 0) {
    for (let parent = this; ; ) {
      for (let { children, positions } = parent._tree, e = dir2 > 0 ? children.length : -1; i != e; i += dir2) {
        ...
        if (next instanceof TreeBuffer) {
          if (mode & IterMode.ExcludeBuffers)
            continue;
          let index = next.findChild(0, next.buffer.length, dir2, pos - start, side);
          if (index > -1)
      ...
      if (parent.index >= 0)
        ...
        i = dir2 < 0 ? -1 : parent._parent._tree.children.length;
...
var BufferNode = class _BufferNode extends BaseNode {
  ...
  child(dir2, pos, side) {
    ...
    let index = buffer.findChild(this.index + 4, buffer.buffer[this.index + 3], dir2, pos - this.context.start, side);
...
var TreeCursor = class {
  ...
  */
  enterChild(dir2, pos, side) {
    ...
    let index = buffer.findChild(this.index + 4, buffer.buffer[this.index + 3], dir2, pos - this.buffer.start, side);
    if (index < 0)
      return false;
  ...
  */
  ...
  atLastNode(dir2) {
    ...
    for (; parent; { index, _parent: parent } = parent) {
      if (index > -1)
        for (let i = index + dir2, e = dir2 < 0 ? -1 : parent._tree.children.length; i != e; i += dir2) {
...
function matchingNodes(node, dir2, brackets) {
  ...
  if (node.name.length == 1) {
    ...
    if (index > -1 && index % 2 == (dir2 < 0 ? 1 : 0))
      return [brackets[index + dir2]];
  }
  return null;
}
...
function matchBrackets(state, pos, dir2, config = {}) {
  let maxScanDistance = config.maxScanDistance || DefaultScanDist, brackets = config.brackets || DefaultBrackets;
  let tree = syntaxTree(state), node = tree.resolveInner(pos, dir2);
  for (let cur = node; cur; cur = cur.parent) {
    let matches = matchingNodes(cur.type, dir2, brackets);
    if (matches && cur.from < cur.to) {
      let handle = findHandle(cur);
      if (handle && (dir2 > 0 ? pos >= handle.from && pos < handle.to : pos > handle.from && pos <= handle.to))
        return matchMarkedBrackets(state, pos, dir2, cur, handle, matches, brackets);
    }
  }
  return matchPlainBrackets(state, pos, dir2, tree, node.type, maxScanDistance, brackets);
}
...
function matchPlainBrackets(state, pos, dir2, tree, tokenType, maxScanDistance, brackets) {
  let startCh = dir2 < 0 ? state.sliceDoc(pos - 1, pos) : state.sliceDoc(pos, pos + 1);
  let bracket2 = brackets.indexOf(startCh);
  if (bracket2 < 0 || bracket2 % 2 == 0 != dir2 > 0)
    return null;
  let startToken = { from: dir2 < 0 ? pos - 1 : pos, to: dir2 > 0 ? pos + 1 : pos };
  let iter = state.doc.iterRange(pos, dir2 > 0 ? state.doc.length : 0), depth = 0;
  for (let distance = 0; !iter.next().done && distance <= maxScanDistance; ) {
    let text = iter.value;
    if (dir2 < 0)
      distance += text.length;
    let basePos = pos + distance * dir2;
    for (let pos2 = dir2 > 0 ? 0 : text.length - 1, end = dir2 > 0 ? text.length : -1; pos2 != end; pos2 += dir2) {
...
var btnAxes = document.getElementById("btnAxes");
var btnWireframe = document.getElementById("btnWireframe");
var btnEdit = document.getElementById("btnEdit");
var jsonEditor = document.getElementById("jsonEditor");
var jsonEditorHost = document.getElementById("jsonEditorHost");
var btnJsonApply = document.getElementById("btnJsonApply");
var btnJsonClose = document.getElementById("btnJsonClose");
var btnJsonFormat = document.getElementById("btnJsonFormat");
var btnJsonExpandAll = document.getElementById("btnJsonExpandAll");
var renderer = new THREE.WebGLRenderer({ canvas, antialias: true, alpha: false });
renderer.setPixelRatio(window.devicePixelRatio || 1);
renderer.outputColorSpace = THREE.SRGBColorSpace;
renderer.setSize(canvas.clientWidth || window.innerWidth, canvas.clientHeight || window.innerHeight, false);
var scene = new THREE.Scene();
scene.background = new THREE.Color(1973790);
var camera = new THREE.PerspectiveCamera(60, 1, 0.01, 4e3);
...
var hemi = new THREE.HemisphereLight(16777215, 16777215, 1.05);
scene.add(hemi);
var dir = new THREE.DirectionalLight(16777215, 0.8);
dir.position.set(1, 2, 1);
scene.add(dir);
var rotatePivot = new THREE.Group();
scene.add(rotatePivot);
var worldRoot = new THREE.Group();
rotatePivot.add(worldRoot);
var jsonHighlightStyle = HighlightStyle.define([
...
var jsonEditorView = new EditorView({
...
function updateHelpers(size) {
  ...
  gridHelper = new THREE.GridHelper(extent, divisions, 6710886, 3355443);
...
function frameToObject(object) {
  ...
  camera.far = Math.max(4e3, distance * 16);
...
async function renderPayload(payload) {
  ...
  const unresolved = (payload.stats?.unresolvedBlocks || []).slice(0, 4).map((u) => `${u.id} (${u.count})`).join(", ");
