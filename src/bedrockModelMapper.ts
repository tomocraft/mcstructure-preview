...
export interface JavaBlockSpec {
  javaId: string;
  state: JavaBlockState;
}

function stateOf(states: Record<string, unknown>, key: string): unknown {
...
function strFrom(value: unknown, fallback = ''): string {
  if (typeof value === 'string') {
    return value;
  }
...
function mirrorNorthSouth(facing: string): string {
  if (facing === 'north') {
    return 'south';
  }
  if (facing === 'south') {
...
function facingFromBedrockDirection(direction: number): string {
  switch (direction) {
    case 2:
      return 'north';
    case 3:
...
function weirdoToFacing(direction: number): string {
  switch (direction) {
    case 0:
      return 'east';
    case 1:
...
function torchFacing(value: string): JavaBlockSpec {
  if (value === 'top') {
    return {
      javaId: 'torch',
      state: {}
...
function slabTypeFromBedrockVerticalHalf(verticalHalf: unknown, blockId: string): JavaBlockSpec {
...
function stairStateFromBedrock(states: Record<string, unknown>): JavaBlockState {
...
function wallSideFromBedrock(value: unknown): string {
...
function standingSignSpec(states: Record<string, unknown>): JavaBlockSpec {
  const woodType = strFrom(stateOf(states, 'wood_type'), 'oak');
  const planksByWood: Record<string, string> = {
    oak: 'oak_planks',
...
export function mapBedrockBlockToJavaModel(
  bedrockId: string,
